...
<div class="container my-5">
    ...
    <div class="card shadow">
        ...
        <div class="card-body text-center">
            @if(isset($ayat))
                <h5 class="mb-3">{{ $ayat['surah']['englishName'] ?? 'Nama Surah' }} (Ayah {{ $ayat['numberInSurah'] ?? '-' }})</h5>
                <p style="font-size: 28px">{{ $ayat['text'] ?? '-' }}</p>
                <p class="text-muted mt-3">{{ $terjemahan['text'] ?? 'Translation not available' }}</p>
            @else
